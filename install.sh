...
# NEUTRON
  ...
  RESTART NOVA
  RESTART NEUTRON
  COMMAND	"sudo ip link set dev br-int up" || echo "failed upping br-int"
  COMMAND	"sudo ip link set dev br-tun up" || echo "failed upping br-tun"
  COMMAND	"sudo ip link set dev br-ex up" || echo "failed upping br-ex"
